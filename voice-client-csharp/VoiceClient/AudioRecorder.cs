...
namespace VoiceClient
{
    ...
    public class AudioRecorder : IDisposable
    {
        ...
        private WaveInProc _callbackDelegate;
        ...
        private static extern int waveInOpen(out IntPtr hWaveIn, int uDeviceID,
            ref WaveFormat lpFormat, WaveInProc dwCallback, IntPtr dwInstance, int fdwOpen);

        [DllImport("winmm.dll")]
        private static extern int waveInClose(IntPtr hWaveIn);

        [DllImport("winmm.dll")]
        private static extern int waveInPrepareHeader(IntPtr hWaveIn, IntPtr lpHeader, int uSize);

        [DllImport("winmm.dll")]
        private static extern int waveInUnprepareHeader(IntPtr hWaveIn, IntPtr lpHeader, int uSize);

        [DllImport("winmm.dll")]
        private static extern int waveInAddBuffer(IntPtr hWaveIn, IntPtr lpHeader, int uSize);
        ...
        private void WaveCallback(IntPtr hWaveIn, int uMsg, IntPtr dwInstance, IntPtr dwParam1, IntPtr dwParam2)
        {
            if (uMsg == MM_WIM_DATA && _isRecording)
            {
                int headerSize = Marshal.SizeOf(typeof(WaveHeader));
                var header = (WaveHeader)Marshal.PtrToStructure(dwParam1, typeof(WaveHeader));
                int bytesRecorded = header.dwBytesRecorded;
                ...
                // 重新加入 buffer
                waveInAddBuffer(_hWaveIn, dwParam1, headerSize);
            }
        ...
        public void Start()
        {
            ...
            int result = waveInOpen(out _hWaveIn, WAVE_MAPPER, ref fmt, _callbackDelegate, IntPtr.Zero, CALLBACK_FUNCTION);
            if (result != 0)
            {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                waveInPrepareHeader(_hWaveIn, _headers[i], headerSize);
                waveInAddBuffer(_hWaveIn, _headers[i], headerSize);
            }
        ...
        public byte[] Stop()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (_headers != null && i < _headers.Length && _headers[i] != IntPtr.Zero)
                {
                    try
                    {
                        var header = (WaveHeader)Marshal.PtrToStructure(_headers[i], typeof(WaveHeader));
                        if (_hWaveIn != IntPtr.Zero)
                            waveInUnprepareHeader(_hWaveIn, _headers[i], headerSize);
