...
namespace VoiceClient
{
    partial class MainForm : Form
    {
        ...
        {
            _cfg = AppConfig.Load();
            _backendUrl = _cfg.VoiceServerUrl;
            Logger.Info($"使用后端地址: {_backendUrl}");
            ...
            _api = new ApiClient(_backendUrl);
        ...
        private async Task StartDebugAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    wav = recorder.Stop();
                    recorder.Dispose();
                    Logger.Info($"麦克风录音完成: {wav?.Length ?? 0} bytes");
                }
                catch (Exception ex)
                {
                    Logger.Warn($"麦克风录音失败: {ex.Message}，使用内置测试语音");
                    // 无麦克风时使用内置测试语音
                    ...
                    {
                        wav = File.ReadAllBytes(testFile);
                        Logger.Info($"使用测试语音: {testFile} ({wav.Length} bytes)");
                    }
                    else
                    {
                        // fallback: 也检查源码目录
                        testFile = "test_voice.wav";
                        if (File.Exists(testFile))
                        {
                            wav = File.ReadAllBytes(testFile);
                        }
